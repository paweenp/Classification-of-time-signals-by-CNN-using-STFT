...
def createCNN(width, height, channel):
    ...
    model.add(keras.layers.Input(shape=(width, height, channel)))  # 88x66 RGB images

    model.add(keras.layers.Conv2D(3, 4 , (1, 1), padding="valid", ))
    ...
    model.add(keras.layers.LocallyConnected2D(3, 2, (1,1)))
...
def train(model, train_images, train_labels, test_images, test_labels, epochs_num, batch_size_num):

    callbacks = [keras.callbacks.ModelCheckpoint("save_at_{epochs_num}.h5"), ]
